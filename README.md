...
This repository contains a Python-based simulation of a **Null Steering Beamformer (NSB)**. The software simulates a 24-element uniform linear antenna array, calculating complex weights to steer the main lobe toward a desired signal while forcing mathematical nulls at the angles of incoming interference signals. To meet strict Side Lobe Level (SLL) constraints ($\leq -20$ dB), it utilizes an iterative peak-nulling algorithm to strategically place artificial "dummy" interferers.

## Features
* **Automated Batch Processing:** Sweeps through hundreds of required SNR and angular ($\delta$) permutations automatically.
* **Data-Driven Configuration:** Uses a central `config.csv` file to control algorithmic boundaries, SNR sweeps, and SLL constraints without altering the core Python source code.
...
* **`optimization.py`**: *The SLL control algorithm.* Implements an iterative, greedy approach that scans the array pattern for the highest side lobe and dynamically drops "dummy" interferers at those exact angles until the overall SLL floor falls below the target.
* **`visualization.py`**: *The plotting backend.* Generates high-resolution 2D plots of the normalized radiation pattern (in dB) and safely saves them directly to disk to prevent memory leaks during massive batch loops.
* **`main.py`**: *The master execution loop.* The entry point for the batch simulation. It reads the configuration parameters, generates the scenario matrix, runs the optimization algorithm, safely saves the `.png` plots to disk, and outputs the final statistical metrics to a text file.
* **`config.csv`**: *The control panel.* A text file dictating the simulation's angular sweeps, SNR values, and algorithmic constraints.

## Configuration

The simulation parameters are separated from the core logic to allow rapid testing. 

### 1. The `config.csv` File
Edit the `config.csv` file in the root directory to adjust the mathematical simulation boundaries:
...
* `SAVE_PLOTS = True`: Generates and saves a `.png` file for every single angular scenario into dynamically created subfolders. *Warning: Increases execution time and consumes disk space due to disk I/O.*
